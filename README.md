...
| `/生成解析` | 无 | 使用 LLM 为最近出示的条目生成详细解析（需先作答/出示答案/复习知识点） |
| `/我的统计` | `[知识库名]` | 查看个人学习统计 |
...
**Q: `/生成解析` 命令怎么用？**
A: 在 `/作答`、`/出示答案` 或 `/开始复习知识点` 之后，直接输入 `/生成解析` 即可。系统会自动获取最近一次出示的条目，使用 LLM 生成详细解析。仅为无解析或解析为空的条目生成。

**注意：** 如果 `/开始复习知识点` 展示了多个条目，`/生成解析` 仅针对最近展示的那一条。
